...
<body>
  <div class="container">
    ...
    <div class="warning">
      <strong>⚠️ Важно:</strong> Чтобы чат-бот смог подключиться к серверу, откройте этот файл не напрямую, а через локальный сервер.
      <br><br>
      Это нужно, чтобы браузер разрешил безопасное подключение к <code>${API_BASE_URL}</code>.
      <br><br>
      <strong>Примеры запуска локального сервера:</strong>
      <div class="code-block">
        # 🐍 Python<br>
        python -m http.server 8000<br><br>

        # 🟢 Node.js<br>
        npx http-server -p 8000 --cors<br><br>

        # 🌐 Ngrok (чтобы протестировать HTTPS)<br>
        ngrok http 8000<br><br>

        После этого откройте:<br>
        👉 <strong>http://localhost:8000/${assistant.name?.replace(/\s+/g, '-').toLowerCase() || 'test-widget.html'}</strong><br>
        или (через ngrok):<br>
        👉 <strong>https://your-ngrok-subdomain.ngrok-free.app/${assistant.name?.replace(/\s+/g, '-').toLowerCase() || 'test-widget.html'}</strong>
      </div>
    </div>

    <p class="subtitle">💬 Чат-бот появится в правом нижнем углу страницы.</p>
    ...
    <div class="info-card">
      <h3>Инструкции:</h3>
      <ol>
        <li>Запустите локальный сервер (см. примеры выше).</li>
        <li>Откройте страницу по адресу <code>http://localhost:8000</code> или HTTPS-ссылке из ngrok.</li>
